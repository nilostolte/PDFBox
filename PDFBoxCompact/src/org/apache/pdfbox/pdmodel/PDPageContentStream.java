...
public final class PDPageContentStream implements Closeable
{
    ...
    /**
     * Outputs a string at the location specified by the current text 
     * matrix, using the correct encoding and subsetting as required.
     * Instead of writing in Hexadecimal ASCII, it writes directly in
     * binary, which is more compact. To be used only with fonts
     * having two bytes GIDs
     * 
     * @param text - The Unicode text to show.
     * @throws IOException
     */
    public void writeTextInbinary(String text) throws IOException {
    	 if (!inTextMode) {
    ...
    protected void showTextInternal(String text) throws IOException
    {
        ...
        byte[] array = font.encode(text);
        
        COSWriter.writeString(array, output);
    }
